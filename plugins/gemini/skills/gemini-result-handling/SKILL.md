...
- For `gemini:gemini-rescue`, do not turn a failed or incomplete Codex run into a Claude-side implementation attempt. Report the failure and stop.
- For `gemini:gemini-rescue`, if Codex was never successfully invoked, do not generate a substitute answer at all.
